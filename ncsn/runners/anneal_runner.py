...
class AnnealRunner():
    ...
    def anneal_Langevin_dynamics(self, x_mod, scorenet, sigmas, n_steps_each=100, step_lr=0.00002, mask = None):
        if mask is None:
            mask = torch.zeros_like(x_mod)
        ...
        with torch.no_grad():
            for c, sigma in tqdm.tqdm(enumerate(sigmas), total=len(sigmas), desc='annealed Langevin dynamics sampling'):
                ...
                for s in range(n_steps_each):
                    ...
                    # x_mod = x_mod + step_size * grad * (1 - mask) 
                    # x_mod = x_mod + step_size * grad * (1 - mask) + noise * (1 - mask)
    ...
    def test(self,epoch):
        ...
        if self.config.data.dataset == 'MNIST':
            ...
            # draw samples from test_loader
            test_X, test_Y = next(iter(test_loader))
            # samples = test_X[:(grid_size ** 2)].to(self.config.device)
            # corrupt samples by column missing
            # randomly select columns to be corrupted
            # rand_col = torch.rand(samples.shape[:-1], device=self.config.device)
            # rand_col = rand_col > 0.5
            # # corrupt samples
            # samples[rand_col] = torch.rand(samples[rand_col].shape, device=self.config.device)
            # mask = torch.ones(samples.shape, device=self.config.device)
            # mask[rand_col] = 0
            samples = test_X[torch.randint(0, test_X.shape[0], (1,))].repeat(grid_size ** 2, 1, 1, 1).to(self.config.device)
            rand_col = torch.rand(samples.shape[-1], device=self.config.device)
            rand_col = rand_col > 0.5
            samples[:, :, :, rand_col] = torch.rand(samples[:, :, :, rand_col].shape, device=self.config.device)
            mask = torch.ones(samples.shape, device=self.config.device)
            ...
            sigmas = np.exp(np.linspace(np.log(0.2), np.log(0.01), self.config.model.num_classes))
            
            all_samples = self.anneal_Langevin_dynamics(samples, score, sigmas, 100, 0.00002, mask = None)
            # all_samples = self.anneal_Langevin_dynamics(samples, score, sigmas, 100, 0.00002, mask=mask)
            ...
            for i, sample in enumerate(tqdm.tqdm(all_samples, total=len(all_samples), desc='saving images')):
                ...
                save_image(image_grid, os.path.join(self.args.image_folder, f'image_{epoch}_{i}.png'))
                torch.save(sample, os.path.join(self.args.image_folder, f'image_raw_{epoch}_{i}.pth'))
